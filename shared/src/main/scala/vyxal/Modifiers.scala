...
object Modifiers {
  val monadicModifiers: Map[String, AST => AST] = Map(
    ...
    "¿" -> (a =>
      Modified(ModifierFunctions.conditionalExecute(a), "¿", Seq(a), 1)
    ),
    "æ" -> (a =>
      Modified(ModifierFunctions.applyToEachStackItem(a), "æ", Seq(a), 1)
    )
  )
  ...
  val dyadicModifiers: Map[String, (AST, AST) => AST] = Map(
    ...
    "]" -> ((a, b) =>
      Modified(ModifierFunctions.ternaryIf(a, b), "]", Seq(a, b), 1)
    )
  )
